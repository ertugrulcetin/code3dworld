...
* [`src/cljs/code3dworld/`](src/code3dworld/renderer/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/code3dworld/renderer/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `code3dworld` project root directory
...
The `debug?` variable in [`config.cljs`](src/code3dworld/renderer/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns code3dworld.example
  (:require [code3dworld.config :as config])
